...
function Reels({
  ...
  const handleMomentumEnd = useCallback(
    (e: any) => {
      if (!focusRef.current) return;
      const index = Math.max(0, Math.round(e.nativeEvent.contentOffset.y / containerHeight));
    ...
    [currentPage, onIndexChange, onNearEnd, safePrefetch, safeVideos.length, containerHeight]
  );
