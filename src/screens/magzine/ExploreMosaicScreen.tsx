...
import { View, Text, StyleSheet, ScrollView, LayoutChangeEvent, TouchableOpacity, StatusBar, RefreshControl, BackHandler } from "react-native";
...
// 🔥 STANDARD MAGAZINE RATIO (Height is ~1.45x the width)
// This ensures the card shape matches the image shape exactly.
const MAGAZINE_RATIO = 1.45;
...
const ExploreMosaicScreen = () => {
  ...
  useEffect(() => {
    ...
    magazines.forEach((m: any) => {
      const cover = m?.media_en?.url || m?.media_te?.url || m?.media?.url;
      if (cover) urls.push(cover);
      const pages = (lang === "te" ? m.pages_te : m.pages_en) || [];
  ...
  const renderCard = (item: Magazine, w: number, h: number) => {
    const title = lang === "te" ? item.title_te : item.title_en || null;
    const imageUrl = lang === "te" ? item.media_te?.url || null : item.media_en?.url || null;
        ...
        <View style={[styles.card, { width: w, height: h }]}>
          {imageUrl ? (
            <FastImage
              ...
              // ✅ CHANGED: Back to 'cover' because we are fixing the container dimensions below.
              // Since container ratio now == image ratio, 'cover' will act like 'contain' but fill edges perfectly.
          ...
          />
          {/* <Text style={[styles.cardTitle, { fontFamily: getFont("regular"), color: Colors.textcolor }]} numberOfLines={2}>
             {item.title} 
          </Text> */}
  ...
  // ✅ CHANGED: Removed the random "heights" array.
  // Instead, we calculate the height based on the width using standard aspect ratio.
  const cardHeight = colW * MAGAZINE_RATIO;
          ...
          >
            <View style={[styles.columnsRow, { gap: GAP }]}>
              <View style={{ width: colW }}>
                {/* ✅ Passing fixed Aspect Ratio Height */}
                {leftItems.map((m) => renderCard(m, colW, cardHeight))}
              </View>
              <View style={{ width: colW }}>
                {/* ✅ Passing fixed Aspect Ratio Height */}
                {rightItems.map((m) => renderCard(m, colW, cardHeight))}
              </View>
            </View>
          </ScrollView>
