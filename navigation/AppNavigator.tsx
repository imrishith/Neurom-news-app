...
const BuzzScreen = React.lazy(() =>
  import("../src/screens/buzz/BuzzScreen")
);
...
const defaultBottomBarItems = [
...

  {
    ...
    label: "Report",
    routeName: "LocationScreen", // fake route, we’ll intercept
...
function InnerStackWithBottomBar({ navReady }: { navReady: boolean }) {
  ...
  useEffect(() => {
    const checkRegistration = async () => {
      try {
        const deviceId = await getStableDeviceId(); // ✅ always stable for you

        console.log("my device_id", deviceId);
